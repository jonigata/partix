			...
			for( int y = y0 ; y <= y1 ; y++ ) {
				for( int x = x0 ; x <= x1; x++ ) {
					...
					if( !active_table_.entry( hv ) ) {
						// optimize: active nodeÇ™Ç»Ç¢Ç∆Ç±ÇÎÇ≈ÇÕè»ó™
						continue;
					}
					...
					// plane - sphere(ÉZÉãïùÇÃëŒäpê¸ÇíºåaÇ∆Ç∑ÇÈ)Ç≈ÉJÉäÉìÉO
...
private:
	...
	public:
			...
			const PenetrationHashNode* p, const FaceHashNode* q ) const
		{
			...
			std::swap( vt::x(uvt), vt::y(uvt) ); // ó†ï\îΩì]ÇµÇƒÇ†ÇÈÇ©ÇÁ
			c_( p->mesh, p->index, q->mesh, q->index, uvt );
			return true;
		}
	...
	private:
			...
			vector_type& uvt ) const
		{
			...
			const face_type& t = q->get_faces()[qq->index];
			const vector_type& v0 = qv[t.i0].new_position;
			const vector_type& v2 = qv[t.i1].new_position; // ó†ï\îΩì]
			const vector_type& v1 = qv[t.i2].new_position; // ó†ï\îΩì]
...
#if 0
		// å¥óùìIÇ…penetration_vectorÇªÇÃÇ‹Ç‹ÇÃí∑Ç≥Ç≈
		// faceÇ…éhÇ≥ÇÈÇ∆ÇÕå¿ÇÁÇ»Ç¢ÇÃÇ≈ÅAêLÇŒÇµÇƒÇ®Ç≠
		// TODO: Ç±ÇÃÉRÅ[Éhì¸ÇÍÇÈÇ∆à¿íËìxÇ™â∫Ç™ÇÈ
